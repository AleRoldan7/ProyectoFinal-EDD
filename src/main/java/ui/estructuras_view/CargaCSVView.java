...
public class CargaCSVView extends VBox {
    ...
    private void cargarSucursales() {
        ...
        try {
            ...
            if (cargadas == 0) {
                ...
                appendLog("[Sucursales] ⚠ No se cargó ninguna sucursal. "
            ...
            appendLog(String.format(
                    "[Sucursales] ✔ %d sucursal(es) en total (%d nuevas). "
                            + "%d nodo(s) registrados en el grafo.",
                    cargadas, nuevas, nodosAgregados));

        } catch (Exception ex) {
            marcarError(estadoSucursales, "Error");
            appendLog("[Sucursales] ✘ Error inesperado: " + ex.getMessage());
            appendLog("             Causa: " + (ex.getCause() != null
                    ? ex.getCause().getMessage() : "desconocida"));
        }
    }
    ...
    private void cargarProductos() {
        ...
        if (state.getCargaCSV().getTotalSucursales() == 0) {
            ...
            appendLog("[Productos] Debes cargar Sucursales.csv primero. "
                    + "Los productos necesitan una sucursal de destino.");
        ...
        try {
            ...
            for (Sucursal s : state.getCargaCSV().getListaSucursales()) {
                ...
                if (cantidad == 0) {
                    sucSinProd++;
                    appendLog(String.format(
                            "[Productos]  Sucursal %d (%s): sin productos asignados.",
            ...
            if (totalProductos == 0) {
                ...
                appendLog("[Productos] ✘ No se asignó ningún producto a ninguna sucursal. "
                        + "Revisa el formato del CSV y que los IDs de sucursal coincidan.");
                return;
            }

            marcarOk(estadoProductos, totalProductos + " productos");
            appendLog(String.format(
                    "[Productos] ✔ %d producto(s) cargados e indexados en el AVL global.",
                    totalProductos));
            mostrarAlerta(Alert.AlertType.CONFIRMATION, "Productos", "Se cargaron lo productos: "+ totalProductos);

            if (sucSinProd > 0) {
                appendLog(String.format(
                        "[Productos]   ⚠ %d sucursal(es) quedaron sin productos "
                                + "(IDs desconocidos o filas con errores).",
                        sucSinProd));
    ...
    private void cargarConexiones() {
        ...
        if (state.getGrafo().getTotalNodos() == 0) {
            ...
            appendLog("[Conexiones] Debes cargar Sucursales.csv primero. "
                    + "El grafo no tiene nodos a los que conectar.");
        ...
        try {
            ...
            if (nodosDespues == 0) {
                ...
                appendLog("[Conexiones] No se cargó ninguna conexión. "
                        + "Verifica el CSV (columnas esperadas: origen,destino,tiempo,costo).");
            ...
            appendLog(String.format(
                    "[Conexiones] Grafo actualizado: %d nodo(s) conectados.",
                    nodosDespues));

            if (nodosDespues < nodosAntes) {
                appendLog(String.format(
                        "[Conexiones]   %d nodo(s) quedaron sin conexiones "
                                + "(sucursales aisladas).",
                        nodosAntes - nodosDespues));
            }
