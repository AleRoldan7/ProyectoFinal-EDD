...
public class ListaEnlazadaView extends VBox {
    ...
    private static final int POR_FILA = 6;
    ...
    public ListaEnlazadaView(AppState state) {
        ...
        Label titulo = new Label("Visualización — Lista Enlazada");
        ...
        btnDibujar.setStyle("-fx-background-color: #2980b9;" + "-fx-text-fill: white; -fx-padding: 6 14;");
        btnRefrescar.setStyle("-fx-background-color: #7f8c8d;" + "-fx-text-fill: white; -fx-padding: 6 14;");

        lblInfo = new Label("Estructura secuencial | Búsqueda: O(n) | " + "Inserción al final: O(n)");
        lblInfo.setStyle("-fx-text-fill: #7f8c8d; -fx-font-size: 11px;");

        HBox controles = new HBox(10, new Label("Sucursal:"), cmbSucursal, btnDibujar, btnRefrescar);
        ...
        PanelExportacion panelExport = new PanelExportacion("Lista Enlazada", () -> {
            if (listaActual == null) return "";
            String sel = cmbSucursal.getValue();
            String nom = sel != null ? "Lista_S" + sel.split(" - ")[0] : "Lista";
            return ExportarEstructuras.listaToDot(listaActual, nom);
        }, (ruta, fmt) -> {
            if (listaActual == null) return false;
            return ExportarEstructuras.exportarListaImagen(listaActual, ruta, fmt);
        });
        ...
        btnDibujar.setOnAction(e -> {
            ...
            lblInfo.setText(String.format("Lista Enlazada: %d productos | " + "O(n) búsqueda | O(1) inserción al inicio", listaActual.size()));
            dibujar(listaActual);
        ...
        this.getChildren().addAll(titulo, new Separator(), controles, lblInfo, scroll, new Label("Datos de la lista:"), tabla, panelExport);
    }

    private void dibujar(ListaEnlazada<Productos> lista) {
        int total = lista.size();
        int filas = (int) Math.ceil((double) total / POR_FILA);
        int cols = Math.min(total, POR_FILA);

        double canvasW = cols * (NODO_W + GAP_X) + PAD_X * 2;
        double canvasH = filas * (NODO_H + GAP_Y) + PAD_Y + 30;
        ...
        gc.fillText("Lista Enlazada: " + total + " nodos", PAD_X, 22);

        gc.setFill(Color.web("#e74c3c"));
        gc.fillRoundRect(PAD_X, 30, 60, 20, 6, 6);
        gc.setFill(Color.WHITE);
        gc.setFont(Font.font("Arial", FontWeight.BOLD, 10));
        gc.fillText("Cabeza", PAD_X + 12, 44);

        if (total > 0) {
            gc.setStroke(Color.web("#e74c3c"));
            gc.setLineWidth(2);
            gc.strokeLine(PAD_X + 60, 40, PAD_X + 80, 40);
            gc.strokeLine(PAD_X + 80, 40, PAD_X + 80, PAD_Y + NODO_H / 2.0);
            gc.strokeLine(PAD_X + 80, PAD_Y + NODO_H / 2.0, PAD_X, PAD_Y + NODO_H / 2.0);
        }
        ...
        while (actual != null) {
            ...
            if (actual.next != null) {
                ...
                if (fila == fila2) {
                    ...
                    double ax1 = x + NODO_W, ay1 = y + NODO_H / 2.0;
                    double ax2 = x + NODO_W + GAP_X / 2.0, ay2 = y + NODO_H + GAP_Y / 2.0;
    ...
    private void dibujarNodo(GraphicsContext gc, Productos p, double x, double y, int idx) {
        gc.setFill(Color.web("#2980b9"));
        gc.fillRoundRect(x, y, NODO_W, NODO_H, 8, 8);

        gc.setStroke(Color.web("#1a6fa0"));
        gc.setLineWidth(1);
        gc.strokeLine(x + NODO_W - 25, y, x + NODO_W - 25, y + NODO_H);
        ...
        if (nom.length() > 13) nom = nom.substring(0, 12) + ".";
        gc.setFill(Color.WHITE);
        gc.setFont(Font.font("Arial", FontWeight.BOLD, 10));
        gc.fillText(nom, x + 4, y + 26);

        String cod = p != null ? p.getBarCode() : "";
        if (cod.length() > 13) cod = cod.substring(0, 12) + ".";
        gc.setFill(Color.web("#d6eaf8"));
        gc.setFont(Font.font(8));
        gc.fillText(cod, x + 4, y + 40);

        gc.setFill(Color.web("#1a6fa0"));
        gc.fillRoundRect(x + NODO_W - 23, y + NODO_H / 2.0 - 6, 18, 12, 3, 3);
        gc.setFill(Color.WHITE);
        gc.setFont(Font.font(7));
        gc.fillText("→", x + NODO_W - 19, y + NODO_H / 2.0 + 4);
    }

    private void dibujarFlecha(GraphicsContext gc, double x, double y, boolean horizontal) {
        gc.setFill(Color.web("#e74c3c"));
        if (horizontal) {
            double[] px = {x, x - 8, x - 8};
            double[] py = {y, y - 4, y + 4};
    ...
    private TableView<Productos> crearTabla() {
        ...
        String[][] cols = {{"#", "idx"}, {"Nombre", "name"}, {"Código", "barCode"}, {"Categoría", "category"}, {"Caducidad", "expiryDate"}, {"Precio", "price"}, {"Stock", "stock"}};

        // Columna índice
        TableColumn<Productos, String> colIdx = new TableColumn<>("#");
        colIdx.setPrefWidth(40);
        colIdx.setCellValueFactory(d -> new javafx.beans.property.SimpleStringProperty(String.valueOf(tabla.getItems().indexOf(d.getValue()))));
        tabla.getColumns().add(colIdx);

        String[][] dCols = {{"Nombre", "name"}, {"Código", "barCode"}, {"Categoría", "category"}, {"Caducidad", "expiryDate"}, {"Precio", "price"}, {"Stock", "stock"}, {"Marca", "brand"}};

        for (String[] dc : dCols) {
            TableColumn<Productos, String> col = new TableColumn<>(dc[0]);
            ...
            col.setCellValueFactory(d -> {
                ...
                String val = switch (campo) {
                    ...
                    case "price" -> String.format("Q%.2f", productos.getPrice());
                    case "stock" -> String.valueOf(productos.getStock());
    ...
    private void cargarTabla(ListaEnlazada<Productos> lista, TableView<Productos> tabla) {
