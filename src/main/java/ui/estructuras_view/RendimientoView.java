...
public class RendimientoView extends VBox {
    ...
    public RendimientoView(AppState state) {
        ...
        btnComparar.setStyle("-fx-background-color: #8e44ad; -fx-text-fill: white;" + "-fx-font-size: 13px; -fx-padding: 7 16;");
        ...
        this.getChildren().addAll(titulo, new Separator(), controles, new Label("Resultados por sucursal y estructura:"), tabla, lblResumen, lblGrafico, canvasBarras, new Separator(), leyenda);
    }
    ...
    private void compararBusquedas(String termino, String selSucursal) {
        ...
        for (Sucursal s : sucursales) {
            ...
            Productos r1 = s.getLista().buscar(p -> p.getName().equalsIgnoreCase(termino));
            ...
            filas.add(new FilaRendimiento(s.getNameSucursal(), ns1, r1 != null, ns2, r2 != null, ns3, r3 != null));
        }

        tabla.setItems(filas);

        int n = sucursales.size();
        lblResumen.setText(String.format("Promedio sobre %d sucursal(es) —  Lista: %,d ns  |  AVL: %,d ns  |  Hash: %,d ns" + "    |    Encontrado en  Lista:%d  AVL:%d  Hash:%d", n, n > 0 ? totalLista / n : 0, n > 0 ? totalAvl / n : 0, n > 0 ? totalHash / n : 0, encontradosLista, encontradosAvl, encontradosHash));

        dibujarBarras(n > 0 ? totalLista / n : 0, n > 0 ? totalAvl / n : 0, n > 0 ? totalHash / n : 0);
    }
    ...
    private TableView<FilaRendimiento> construirTabla() {
        ...
        colGanador.setCellFactory(tc -> new TableCell<>() {
            ...
            protected void updateItem(String v, boolean empty) {
                ...
                setText(v);
                setStyle(v.contains("Hash") ? "-fx-background-color: #d5f5e3; -fx-font-weight: bold;" : v.contains("AVL") ? "-fx-background-color: #fef9e7; -fx-font-weight: bold;" : "-fx-background-color: #fdecea; -fx-font-weight: bold;");
            }
        });
        colGanador.setPrefWidth(100);

        tv.getColumns().addAll(colSuc, colListaNs, colListaRes, colAvlNs, colAvlRes, colHashNs, colHashRes, colGanador);

        tv.setRowFactory(r -> new TableRow<>() {
            @Override
            protected void updateItem(FilaRendimiento item, boolean empty) {
                super.updateItem(item, empty);
                if (empty || item == null) {
                    setStyle("");
                } else {
                    setStyle(getIndex() % 2 == 0 ? "" : "-fx-background-color: #f8f9fa;");
                }
            }
        });
    ...
    private TableColumn<FilaRendimiento, String> colNum(String titulo, String prop) {
        ...
        col.setCellValueFactory(d -> {
            switch (prop) {
                case "listaNs":
                    return d.getValue().listaNsProp();
                case "avlNs":
                    return d.getValue().avlNsProp();
                case "hashNs":
                    return d.getValue().hashNsProp();
                default:
    ...
    private TableColumn<FilaRendimiento, String> colRes(String titulo, String prop) {
        ...
        col.setCellValueFactory(d -> {
            switch (prop) {
                case "listaRes":
                    return d.getValue().listaResProp();
                case "avlRes":
                    return d.getValue().avlResProp();
                case "hashRes":
                    return d.getValue().hashResProp();
                default:
                    return new SimpleStringProperty("—");
            }
        });
        col.setCellFactory(tc -> new TableCell<>() {
            ...
            protected void updateItem(String v, boolean empty) {
                ...
                setStyle("Bien".equals(v) ? "-fx-text-fill: #27ae60; -fx-font-weight: bold;" : "-fx-text-fill: #e74c3c;");
            }
        });
        col.setPrefWidth(80);
        return col;
    ...
    private VBox crearLeyenda() {
        ...
        String[][] rows = {{"Lista enlazada", "O(n)", "Recorre todos los nodos hasta encontrar el elemento"}, {"Árbol AVL", "O(log n)", "Divide el espacio de búsqueda en cada paso"}, {"Tabla Hash", "O(1) prom", "Acceso directo por función hash (ideal)"}, {"Árbol B", "O(log n)", "Índice por fecha de caducidad, óptimo para rangos"}, {"Árbol B+", "O(log n)", "Índice por categoría, hojas enlazadas para recorrido"},};
        for (String[] r : rows) {
    ...
    public static class FilaRendimiento {
        ...
        public SimpleStringProperty sucursalProp() {
            return sucursal;
        }

        public SimpleStringProperty listaNsProp() {
            return listaNs;
        }

        public SimpleStringProperty listaResProp() {
            return listaRes;
        }

        public SimpleStringProperty avlNsProp() {
            return avlNs;
        }

        public SimpleStringProperty avlResProp() {
            return avlRes;
        }

        public SimpleStringProperty hashNsProp() {
            return hashNs;
        }

        public SimpleStringProperty hashResProp() {
            return hashRes;
        }

        public SimpleStringProperty ganadorProp() {
            return ganador;
        }
    }
}